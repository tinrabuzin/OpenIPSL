...
package PowerFactory
  package ElmModels
    ...
    equation
      ...
      p.ir = 0;
      annotation(
        Icon(graphics = {Rectangle(fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid, extent = {{-100, 100}, {100, -100}}), Text(origin = {0, 90}, extent = {{-100, 10}, {100, -10}}, textString = "StaVmeas"), Text(origin = {0, 50}, extent = {{62, 8}, {100, -10}}, textString = "u"), Text(origin = {0, -50}, extent = {{62, 8}, {100, -10}}, textString = "fe")}));
    end StaVmeas;
  end ElmModels;
  ...
  package WECC
    ...
    equation
      ...
      p.ir = 0;
      annotation(
        Diagram(graphics = {Rectangle(fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid, extent = {{-100, 100}, {100, -100}}), Text(origin = {-15, 93}, extent = {{-85, 7}, {115, -13}}, textString = "ElmPhi_pll")}));
    end ElmPhi_pll;
...
// Tripping for high values of the input signal
      if u > Lv3 then
        TrpHigh = 0;
      elseif u > Lv2 then
        TrpHigh = (Lv3 - umax + (if u >= umax then 0 else recov * (umax - u))) / (Lv3 - Lv2);
      else
        TrpHigh = if umax <= Lv2 then 1.0 else (Lv3 - umax + recov * (umax - Lv2)) / (Lv3 - Lv2);
      end if;
      annotation(
